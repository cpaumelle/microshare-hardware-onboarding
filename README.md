...
### [`examples/dot/`](examples/dot/) — Inbound webhook (pest trap, partner-facing brief)

DOT-style pest trap vendor. POSTs trap event JSON to a Microshare pipe-token URL; downstream Robots enrich, alert, and bundle into customer-visible incidents. This folder is the "talk-to-the-vendor" brief plus implementation plan — Robot code is added once a real payload sample is captured. See the [Taqt example](examples/taqt/) for the same webhook pattern with worked code.
...
> **Deploying the Robot?** Its `data.auth` must be a `grant_type=robot` token (scope
> `SHARE:READ,QUERY,WRITE`; 5 scopes for a bundler) that returns 200 on `/api/share`.
> An `ALL:ALL`/session token 401s there and the robot writes nothing. See
> [Robot auth](reference/composer-api.md#robots).
...
```bash
python tools/validate.py --rectype io.microshare.trap.packed
```

(Avoid `io.microshare.feedback.*` for this check — its event-meta listings are
platform-restricted under most identities and can return 0 even when records exist.)
...
# Read the latest records (always bound a listing with since= or it 504s on volume)
curl -s "https://dapi.microshare.io/share/io.myapp.events?details=true&since=7d" \
  -H "Authorization: Bearer $TOKEN" | jq ".objs[].data"
```

> Hosts above are **dev** (`dapi`). On prod use `pest.microshare.io/api` for pest-vanity
> tenants — `api.microshare.io` silently returns a subset of records for those.
...
| [`reference/composer-api.md`](reference/composer-api.md) | Microshare Composer API — auth, data lake, **Robots (incl. robot-auth)**, device clusters |
