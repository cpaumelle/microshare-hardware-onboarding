 ...
 *     (consumed by feedback dashboards/forms that read the standard
 *     feedback event format)
 *
 * Emits one event.meta record per detected press. Detection rules (mirroring
 * the platform's Smilio event handler):
 *
 ...
 * (the backboard is a VIEW, not a share record). The backboard
 ...
 *
 * Note: this robot does NOT also write io.microshare.event.alert.feedback. That
 * is a separate alert-emitter robot's job, which applies notification rules.
 * The platform writes that alert directly; splitting the two lets a rules layer
 * filter alerts before they notify (e.g. suppress positive feedback).
 */
...
// Tags on emitted event.meta records — match the standard positional pattern
// so a feedback form's positional location filters (loc1, loc2, ...)
// resolve correctly:
//
