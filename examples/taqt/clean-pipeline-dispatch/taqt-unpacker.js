 ...
 * SmilioAction decoder produces (see SmilioAction.scala in the public
 * microshare/lib-unpackers repo), so a Taqt-fed cluster's records are
 * readable by every downstream consumer that reads Smilio-fed records:
 *
 *   - Platform handlers (SmilioEventHandler, the Scala decode chain, the ES
 *     Pest Bundler) fire on LoRaWAN/platform-fed records ONLY — NOT on
 *     records a robot or pipe token writes. A robot-fed pipeline must produce
 *     its own derived records (this file's companion event-meta.js); don't
 *     rely on the platform. So although these unpacked records are pixel-
 *     identical to Smilio's, the platform's SmilioEventHandler does NOT diff
 *     them or emit io.microshare.taqt.unpacked.event.meta /
 *     io.microshare.event.alert.feedback from them — event-meta.js writes
 *     those derived records precisely because the platform doesn't.
 *   - Feedback forms that read the standard Smilio shape work directly
...
// Prod host: pest-vanity tenants MUST use https://pest.microshare.io/api. api.microshare.io
// authenticates but returns only a SUBSET of records for them (use it only for generic accounts).
...
function loadTwinLookup(auth) {
    ...
    try {
        ...
        for (var i = 0; i < result.body.objs.length; i++) {
            ...
            if (!dcMeta) {
                // Mirror Smilio's meta.dc shape exactly. The deviceUnpacker string is
                // the Smilio decoder name, so the cluster looks like a real
                // Smilio cluster to every downstream consumer of the
                // .event / .event.meta / .alert.feedback records.
                dcMeta = {
            ...
            //   3. clusterData.meta.location ("a,b,c" CSV — the standard location
            //      pattern the platform pipeline also reads. Parsing it here makes
            //      this robot's records match the structure of LoRaWAN-pipeline
            //      records, so a feedback form's default view can filter on
            //      data.meta.global $all dataContext.)
...
// the platform's Smilio event handler, feedback forms, and backboard
// config all join against.
//
...
function main(text, auth) {
    ...
    try {
        ...
        // Field-for-field cross-checked against the Smilio decoder output
        // (SmilioAction.scala in the public microshare/lib-unpackers repo).
        var dcMeta = bindings.dcMeta || {
            id: '',
            name: '',
            network: 'com.taqt.ubiqod',
            recType: PACKED_RECTYPE,
            // Defaults to the Smilio decoder name so the record matches a real
            // Smilio cluster's shape even if the cluster has no explicit
            // unpacker configured yet.
        ...
        for (var ri = 0; ri < sensorRecords.length; ri++) {
            ...
            var unpacked = {
                ...
                origin: {
                    // Convention: vendor data lives under origin.<vendor>.
                    ubiqod: {
