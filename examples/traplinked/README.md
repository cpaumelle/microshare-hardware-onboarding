...
2. **Twin**: Robot reads the device cluster via `httpGet` to `/api/device/` endpoint for location tags
...
6. **Log**: `lib.writeShare(auth, 'io.microshare.traplinked.log', ...)` for each poll stage
...
| `meta.dc` | Cluster metadata: `network: "com.traplinked"`, `usecase: "trap"` |
...
| `origin.traplinked` | Full Traplinked API response preserved for debugging |
...
5. `MS_API_HOST` is `https://pest.microshare.io` — change to `https://api.microshare.io` for non-pest environments
   - Prod host: pest-vanity tenants MUST use `https://pest.microshare.io/api`. `api.microshare.io` authenticates but returns only a SUBSET of records for them (use it only for generic accounts).
6. Deploy as a scheduled robot (60s interval) via `src/deploy-atom.py`
...
The robot writes a log record to `io.microshare.traplinked.log` at each stage (`twins`, `unpacked`, `poll`, `error`). Query these in Composer or via the API to debug.
