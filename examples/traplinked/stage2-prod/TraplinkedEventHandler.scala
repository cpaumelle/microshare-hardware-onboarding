 ...
 *
 * Unlike a motion-sensor rodent handler (which uses 4-reading motion pattern
 * analysis for passive PIR sensors), this handler processes definitive event
 * types from the Traplinked API: catch_detected, trap_triggered, etc.
 *
 * Routing: the platform routes records to this handler by decoder name
 * "com.traplinked.trap.JERRY.Decoder" (not by use_case SC05).
 *
