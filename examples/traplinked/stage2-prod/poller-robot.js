...
function main(text, auth) {
    ...
    for (var i = 0; i < reports.length; i++) {
        ...
        // Write packed record for the platform path. NOTE: this robot write does
        // NOT itself trigger device-cluster decode — decode runs on real network-server
        // ingestion, not on robot/pipe writes to a packed recType. The Scala decoder
        // unpacks records that arrive via the platform NS path, not these robot writes.
