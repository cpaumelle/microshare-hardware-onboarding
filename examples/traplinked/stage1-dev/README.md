...
1. **Create a device cluster** in Composer on `io.microshare.traplinked.packed`
   ...
   - Commas are allowed ONLY in the cluster's `meta.location` CSV string (the platform splits it into global tags). Never put commas in actual tag values / path segments — that breaks the bundler.
