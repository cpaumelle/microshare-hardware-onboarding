...
var LOG_RECTYPE    = 'io.microshare.traplinked.log';
...
function log(auth, stage, data) {
    print(stage + ': ' + JSON.stringify(data));
    try {
        lib.writeShare(auth, LOG_RECTYPE,
            { robot: 'traplinked-poller', stage: stage, data: data, ts: new Date().toISOString() },
            ['traplinked', 'log', stage]);
    } catch (e) {}
}

// NOTE: httpGet is duplicated across hardware robot files (traplinked, futura).
// Microshare Robots run in an isolated GraalJS context with no module system,
// so shared helpers cannot be imported. Any change here must be replicated.
...
// Two things to know:
//   1. Device clusters are Composer objects (under /device/), not share
//      records, so neither share-API helper would work for them anyway.
//   2. lib.readShareByType does NOT exist as a lib function. Calling it
//      crashes the JS context silently — robot fires but writes no output
//      and no logs. For share-record reads use lib.readShareByTags(auth,
//      recType, [], {}) instead.
// The auth parameter passed to main() is a valid Bearer token for the
// Microshare API, so we call /device/ directly via Java HttpURLConnection
// (same as the Composer UI does).
...
function loadTwinLookup(auth) {
    ...
    try {
        ...
        log(auth, 'twins', { count: Object.keys(lookup).length });
    } catch (e) {
...
function main(text, auth) {
    ...
    if (result.status !== 200 || !result.body) {
        log(auth, 'error', { message: 'API failed', status: result.status });
        return bindings;
    }
    ...
    for (var i = 0; i < reports.length; i++) {
        ...
        while (bindings.seenReports.length > 500) bindings.seenReports.shift();
    ...
    if (newCount > 0) {
        log(auth, 'poll', { device: TL_DEVICE_ID, new_events: newCount });
    }
