// Composer permissions: Share Read, Share Query, Share Write
// (a forwarder/alert-generator needs only these 3; the 5-scope set
//  READ,QUERY,WRITE,EXECUTE,POLICY is for bundlers, not this robot)
/*
 ...
 *
 * Platform-fires note: the platform pest incident bundler may not create incidents
 * on a robot-fed pipeline (its trigger is tied to the platform ingestion path).
 * If incidents don't appear, ship a direct-write JS bundler instead of relying
 * on the platform bundler to consume these robot-written alerts.
 *
...
// ── Build alert record ────────────────────────────────────────────────────
//
// Matches the standard rodent alert format the platform produces
// so the pest incident bundler can process it identically.
