...
# Prod default is the generic host. PEST-VANITY TENANTS MUST override with
# MICROSHARE_API=https://pest.microshare.io/api — api.microshare.io authenticates
# but silently returns only a SUBSET of records for those tenants.
API_HOSTS = {"dev": "https://dapi.microshare.io", "prod": "https://api.microshare.io"}
API = os.getenv("MICROSHARE_API") or API_HOSTS.get(ENV, API_HOSTS["dev"])
...
def main():
    ...
    try:
        ...
        # Don't assert a length: prod tokens are 64-char hex but dev now issues
        # JWT-format tokens (no fixed length). A non-empty token + the API calls
        # below are the real proof it works.
        all_ok &= check("Authentication", bool(token), f"token {token[:8]}...")
    ...
    # 4. Data Lake access
    try:
        # Always pass `since=` on a high-volume recType — an unbounded listing 504s.
        r = requests.get(
            f"{API}/share/io.microshare.openclose.unpacked",
            headers=headers,
            params={"limit": "1", "since": "7d"},
            timeout=30,
        )
    ...
    if args.rectype:
        ...
        try:
            ...
            if clusters:
                ...
                info("A device cluster is REQUIRED for decode + location twinning —")
                info("it is not optional. Decode runs only via the cluster's network")
                info("server (EXTRACT -> MATCH -> MAP); an unpacker robot reads the")
                info("cluster for meta.device. No cluster -> no twinning, and a")
                info("decode robot has nothing to resolve devices against.")
                info("Create a cluster in Composer on this recType and register")
                info("your devices (IDs + location tags).")
        except Exception as e:
