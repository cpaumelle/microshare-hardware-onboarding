...
|---|---|---|---|
| `{ingestHost}` | Microshare API endpoint | `dapi.microshare.io` | `pest.microshare.io/api` † |
| `{recType}` | Your packed recType | `io.microshare.trap.packed` | `io.microshare.trap.packed` |
| `{pipeToken}` | Write-only token | generated below | generated below |

† **Pest-vanity tenants must use `pest.microshare.io/api`.** `api.microshare.io`
authenticates but silently returns a **subset** of records for those tenants — use it
only for generic (non-pest) accounts. Pipe **tokens** are 64-char hex on prod; dev
`dauth` now issues JWT-format pipe tokens (permanent, write-only, no expiry).
...
Request a token with scope `SHARE:WRITE` — a write-only token suitable as a pipe token. (Use scoped tokens, not `ALL:ALL`; a pipe token only needs write.)
...
For production, use `pest.microshare.io/api` (pest tenants) instead of `dapi`. See the host note above before using `api.microshare.io`.
...
Once data is flowing to the packed recType, [deploy a Robot](../examples/traplinked/README.md) to unpack it into Microshare's standard schema. See also [Robots in the API reference](composer-api.md#robots) — note the `grant_type=robot` auth requirement.
...
- For dev, use `dapi.microshare.io`. For prod, use `pest.microshare.io/api` (pest-vanity tenants) — see the host note at the top before reaching for `api.microshare.io`. Do **not** use `dingest` / `ingest` hostnames: records written via those land in the LoRaWAN ingest tier, are stored separately, and cannot be read back through the Share API or Views.
- A pipe-token write to a packed recType triggers your unpacker **robot** (the flow above). It does **not** trigger device-cluster **decode** — that runs only on the real network-server ingestion path, so you cannot validate a cluster's decoder by POSTing packed records here.
